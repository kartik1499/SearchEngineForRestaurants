...
public class Cosine_Similarity {
	 ...
	 public static void main(String[] args)
	 {
	  ...
	  double sim_score = cs1.Cosine_Similarity_Score("travel happiness family New York Los Angeles", "science knowledge travel San Diego");
	  System.out.println("Cosine distance score = "+sim_score);
	  ArrayList<String> list = new ArrayList<String>();
	  list.add("travel happiness family New York Los Angeles");
	  list.add("medicine biology university cells San Diego");
	  list.add("science knowledge travel San Diego");
	  list.add("mother girl child family New York");
	  list.add("singing dancing shopping shopping singing shopping New York");
	  list.add("fitness beach Los Angeles San Diego");
	  list.add("Computers medicine university internet family");
	  list.add("chemistry substance science nature family");
	  list.add("Medicine New York Los Angeles");
	  list.add("Science family travel shopping");
	  String s="";
	  for(int i=0;i<list.size();i++){
		  s+="{";
		  for(int j=0;j<list.size();j++){
			  double val = cs1.Cosine_Similarity_Score(list.get(i),list.get(j));
			  double round = round(val, 4);
			  s+=""+round+",";
		  }
		  s+="},";
	  }
